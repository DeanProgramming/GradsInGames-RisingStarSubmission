...
public class Environment : MonoBehaviour
{
    ...
    private readonly Vector3 NodeSize = Vector3.one * 4.0f; //9.0f;
    private const float TileSize = 5.0f;// Changed from 10.0f; I want the grid to be smaller
    ...
    public void CleanUpWorld()
    {
        ...
        foreach (Character item in FindObjectsOfType<Character>())
        {
            if (item != null)
            {
        ...
        foreach (CollectablesBaseClass item in FindObjectsOfType<CollectablesBaseClass>())
        {
        ...
        foreach (KeyScript item in FindObjectsOfType<KeyScript>())
        {
            if (item.gameObject != null)
            {
                Destroy(item.gameObject);
    ...
    public List<EnvironmentTile> Solve(EnvironmentTile begin, EnvironmentTile destination)
    {
        ...
        if (begin != null && destination != null)
        {
            ...
            {
                //Change the original code to add a check to see if destination is accessible
